...
def eternity_convert(file, config, output, provider_file_enabled=True):
    ...
    all_provider = subs_function.convert_sub(
        "https://raw.githubusercontent.com/SSgumS/V2RayAggregator/master/sub/sub_merge_base64.txt",
        "clash",
        "http://0.0.0.0:25500",
        False,
    ...
    """
    config_yaml = subs_function.dump_yaml(config)
...
def backup(file):
    try:
        ...
        print(
            "Error While backup EterniyBase_file => if you use method yaml ignore this"
        )
