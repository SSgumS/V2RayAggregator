...
class subs_function:
    ...
    def convert_sub(
        url: str,
        output: str,
        convertor_host="http://0.0.0.0:25500",
        show_url=False,
        extra_options="",
    ):
    ...
    def fix_proxies_name(corresponding_proxies: []):
        ...
        for index, c_proxy in enumerate(corresponding_proxies):
            ...
            # add exclude list
            if (
                country_code in exclude_list_of_countries
                or name_emoji == emoji["NOWHERE"]
            ):
    ...
    def fix_proxies_duplication(corresponding_proxies: []):
        ...
        while begin < length:
            ...
            while begin_2 <= (length - 1):
                ...
                if (
                    proxy_compared["server"] == correspond_next_proxy["server"]
                    and proxy_compared["port"] == correspond_next_proxy["port"]
                ):
                    check = True
                    ...
                    if (
                        "ws-opts" in correspond_next_proxy
                        and "ws-opts" in proxy_compared
                    ):
                        if (
                            proxy_compared["ws-opts"]
                            != correspond_next_proxy["ws-opts"]
                        ):
                            check = False
                    ...
                    if (
                        "network" in correspond_next_proxy
                        and "network" in proxy_compared
                    ):
                        if (
                            proxy_compared["network"]
                            != correspond_next_proxy["network"]
                        ):
                            check = False
