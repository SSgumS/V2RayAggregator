        ...
        if "</b>" not in sub_content:
            if "proxies:" not in sub_content:  # 对 URL 内容进行格式化处理
                ...
                try:
                    ...
                    try:
                        # fix compatability with Clash
                        sub_content = re.sub(
                            sub_convert.password_pattern,
                            sub_convert.replace_passwords,
                            sub_content,
                        )
        ...
        if dup_rm_enabled:  # 去重
            ...
            while begin < length:
                ...
                while begin_2 <= (length - 1):
                    check = False
                    if (
                        proxy_compared["server"] == proxies_list[begin_2]["server"]
                        and proxy_compared["port"] == proxies_list[begin_2]["port"]
                    ):
                        check = True
                        ...
                        if (
                            "password" in proxies_list[begin_2]
                            and "password" in proxy_compared
                        ):
                            if (
                                proxy_compared["password"]
                                != proxies_list[begin_2]["password"]
                            ):
                                check = False

                        if (
                            "cipher" in proxies_list[begin_2]
                            and "cipher" in proxy_compared
                        ):
                            if (
                                proxy_compared["cipher"]
                                != proxies_list[begin_2]["cipher"]
                            ):
                                check = False
        ...
        try:
            ...
            for line in lines:
                try:
                    ...
                    if "vmess://" in line:
                        try:
                            ...
                            if vmess_config["id"] == "" or vmess_config["id"] is None:
                                ...
                                yaml_url.setdefault(
                                    "name",
                                    urllib.parse.unquote(str(vmess_config["ps"])),
                                )
                                ...
                                if (
                                    ...
                                    or vmess_config["net"] == "h2"
                                    or vmess_config["net"] == "grpc"
                                ):
                    ...
                    if (
                        "ss://" in line
                        and "vless://" not in line
                        and "vmess://" not in line
                    ):
                        if "#" not in line:
                            line = line + "#SS%20Node"
                        try:
                    ...
                    if "ssr://" in line:
                        try:
                            ...
                            yaml_url.setdefault(
                                "password",
                                sub_convert.base64_decode(password_encode_str),
                            )
                            yaml_url.setdefault("obfs", parts[4])
                            yaml_url.setdefault("protocol", parts[2])
                            yaml_url.setdefault(
                                "obfsparam",
                                sub_convert.base64_decode(param_dic["obfsparam"]),
                            )
                            yaml_url.setdefault(
                                "protoparam",
                            ...
                            if (
                                ...
                                if "headers" in proxy_config["ws-opts"]:
                                    if proxy_config["ws-opts"]["headers"]["Host"] != "":
                                        vmess_value["host"] = proxy_config["ws-opts"][
                                            "headers"
                                        ]["Host"]

                                if "path" in proxy_config["ws-opts"]:
                                    if proxy_config["ws-opts"]["path"] != "":
                                        vmess_value["path"] = proxy_config["ws-opts"][
                                            "path"
                                        ]
    ...
    def convert_remote(url="", output_type="clash", host="http://127.0.0.1:25500"):
        ...
        if output_type == "clash":
            converted_url = (
                ...
                + url
                + "&insert=false&emoji=true&list=true"
            )
            try:
