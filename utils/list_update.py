...
class update_url:
    def update_main(
        ...
        for sub in raw_list:
            ...
            if use_airport == False:
                if (
                    airports_id.__contains__(id) == False
                    and sub["update_method"] != "update_airports"
                ):
                    try:
        ...
        if id == 5:
            ...
            urllist = list(
                set(
                    list(
                        filter(
                            lambda x: x != "" and str(x).startswith("http"),
                            s.get(
            ...
            # air_free_2 = list(set(list(filter(lambda x: x != "" and str(x).startswith("http"), s.get(
    ...
    def change_date(id, current_url):
        if id == 0:
            ...
            url_front = (
                "https://raw.githubusercontent.com/pojiezhiyuanjun/freev2/master/"
            )
            url_end = ".txt"
        ...
        if id == 2:
            try:
                res_json = requests.get(
                    "https://api.github.com/repos/mianfeifq/share/contents/"
                ).json()
